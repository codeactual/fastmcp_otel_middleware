...
class MockToolCallMessage:
    ...
    def __init__(
        self, name: str, arguments: dict | None = None, meta: dict | None = None
    ):
        self.name = name
...
async def test_attribute_export():
    ...
    print(
        "(These appear in Langfuse under metadata.attributes - VISIBLE but NOT queryable)"
    )
    ...
    if langfuse_attrs:
        ...
        print(
            "(These appear in Langfuse as top-level metadata fields - VISIBLE AND queryable)"
        )
...
""")
    ...
    export_data = {
        ...
        "metadata": {
            "attributes": {
                k: v for k, v in standard_attrs.items()
            },
...
if __name__ == "__main__":
    ...
    asyncio.run(test_attribute_export())
